...
def upload_file():
    ...
    try:
        ...
        result = subprocess.run(
            ['python', 'pacing_plan.py', '-f', file_path, '-t', time, '-p', paces] + ([is_loop] if is_loop else []),
            capture_output=True,
            text=True
        )
        print(result)
        ...
        output_json_path = os.path.join('..', 'perfect_pace_data', base_filename, f"{time}min_{paces}p.json")
        output_txt_path = os.path.join('..', 'perfect_pace_data', base_filename, f"{time}min_{paces}p.txt")

        # Assume that both files are generated by the script
        if not os.path.exists(output_json_path) or not os.path.exists(output_txt_path):
    ...
    #print(f"Checking if files exist: {output_json_path}, {output_txt_path}")
    #print(f"Output JSON exists: {os.path.exists(output_json_path)}")
    #print(f"Output TXT exists: {os.path.exists(output_txt_path)}")
    #return jsonify({'cool': 'hi'}), 200
